class Analytics
    ...
    Question.collection.aggregate([
      ...
      { "$group": 
        { "_id": group_by, 
          ...
          "male_votes": {"$sum": {"$cond": [ { "$eq": [ "$asker_gender", "male" ] }, "$requestor_count", 0 ]}}, 

          "female_votes": {"$sum": {"$cond": [ { "$eq": [ "$asker_gender", "female" ] }, "$requestor_count", 0 ]}}, 

          "other_votes": {"$sum": {"$cond": [ { "$eq": [ "$asker_gender", "other" ] }, "$requestor_count", 0 ]}}, 
  ...
  def getDataCount (data = {})
    ...
    data[:votes] = questions.sum(:requestor_count) rescue 0
    data[:voting_region] = questions.group_by(&:state_name).map{|state,quests| ({state => quests.sum(&:requestor_count)}) }.reduce({}, :merge).max_by{|name,count| count}.first.to_s rescue nil
  ...
  def question_info
    match_by = if @filter[:from]=="infinite"
        {
    ...
    @q_info = Tracking.collection.aggregate([
          ...
          { "$group": 
            { "_id": "null",
              ...
              "other_votes": {"$sum": {"$cond": [ { "$eq": [ "$actor_gender", "other" ] }, 1, 0 ]}},
              "total_votes": { "$sum": 1 },
  ...
  def getQuestionData
    ...
    @data = Tracking.collection.aggregate([
          { "$match": match_by },
          { "$group": 
            { "_id": group_by,
              "male_votes": {"$sum": {"$cond": [ { "$eq": [ "$actor_gender", "male" ] }, 1, 0 ]}},
  ...
  def sanitize key
    ...
    when :by
      %w(date geolocation).include?(@filter[key].to_s) ? @filter[key].to_s : 'date'
    when :offset
      if @filter[:questions_only]
    ...
    when :sort_by
      %w(date_desc date_asc vote_desc vote_asc last_vote_desc last_vote_asc sentmnt_asc sentmnt_desc).include?(@filter[key].to_s) ? sort_by_value(@filter[key]) : 'created_at DESC'
    when :sentiment_from
